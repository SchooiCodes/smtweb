...
- Write release notes in `../smt/updatelogs.txt` - the site renders each blank-line-separated block as one release card: first line bold as the title, the rest as detail lines below. Keep writing it commit-message style (subject + body). Releases render first, then the site backfills with recent `smt` commit subjects (deduped, max 8 total, commit items get a date label) so the changelog never looks empty. That file gets overwritten per release, so the backfill is what keeps history visible.
